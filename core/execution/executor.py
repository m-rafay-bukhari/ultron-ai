    ...
    async def execute_tool(self, request: ToolExecutionRequest) -> ToolExecutionResult:
        ...
        try:
            ...
            raise ToolNotFoundException(
                f"Tool {tool_name} not found or could not be loaded"
            )
        ...
        if perm_level == PermissionLevel.DENY:
            raise PermissionDeniedException(
                f"Execution of tool {tool_name} is denied by security policy"
            )
            ...
            if not allowed:
                raise PermissionDeniedException(
                    f"User denied permission to execute tool {tool_name}"
                )
